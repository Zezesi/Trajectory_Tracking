...
if __name__ == "__main__":
    ...
    for i in range(N+1):
        for j in range(len(X_obs)):
            collision_constraints = ca.sqrt((veh_states[i, 0]-X_obs[j])**2+(veh_states[i, 1]-Y_obs[j])**2) - safe_diam/2
            opti.subject_to(opti.bounded(0.0, collision_constraints, math.inf))
